...
- **Feature 3 — Resposta Automática de Perguntas** (completa): cliente real da API do ML em `backend/src/integrations/mercadolivre/mlApiClient.ts` (buscar perguntas não respondidas, buscar item/descrição, enviar resposta via `/answers`). Coletor `backend/src/jobs/questionCollector.job.ts` faz polling a cada 5 min, dedup por unique `tenantId+platform+externalId`, e enfileira `questionAi.job.ts` (sugestão da IA). Pipeline `backend/src/services/ai/questionAnswer.pipeline.ts` classifica + responde com contexto (título/descrição), retorna `confianca` 0-1 e `requer_revisao_humana` (sem cache; modelo `claude-sonnet-4-6`). Auto-envio só com regra de IA ativa + confiança ≥ 0.9 + sem flag de revisão. Endpoints: `GET /api/questions?status=`, `POST /api/questions/:id/approve`, `PATCH /api/questions/:id` (editar+enviar), `POST/GET /api/listings/:id/auto-replies`. Inbox em `frontend/src/app/(dashboard)/questions/` com abas pendentes/respondidas, ConfidenceBadge e aprovar/editar.
  - **Gap conhecido (futuro):** OAuth do ML por tenant não existe — `mlCredentials.ts` resolve token via env `ML_ACCESS_TOKEN`/`ML_SELLER_ID` (dev). A entrega final de notificações (`notificationQueue`) também é feature futura.
- **Feature 4 — Dashboard de Rentabilidade Real** (completa): cálculo determinístico em `backend/src/services/profitability.calc.ts` (lucro líquido, margem %, ponto de equilíbrio, flag de prejuízo — a IA NÃO faz conta). Custos em `product_costs` (1 por anúncio), série de margem em `profitability_snapshots`. Pipeline `backend/src/services/ai/profitability.pipeline.ts` gera só o texto de análise+recomendação+urgência (cache 24h, `claude-sonnet-4-6`), via fila `profitabilityAi.job.ts`. Endpoints: `POST /api/listings/:id/costs`, `GET /api/listings/:id/profitability` (números + histórico), `GET /api/profitability` (todos ordenados por margem), `POST /api/listings/:id/profitability/analyze` + `GET .../analyze/:jobId` (IA via polling). Tela `frontend/src/app/(dashboard)/profitability/`: tabela por margem (prejuízo destacado), CostForm, ProfitabilityCard com quebra de custos, MarginChart (recharts) e alerta visual de prejuízo.
- **Feature 5 — Detector de Queda de Vendas** (completa): detecção determinística em `backend/src/services/salesTrend.calc.ts` (compara últimos 30d vs 30d anteriores, `declinePercent`, `hasDrop` com limiar 25% — a IA NÃO decide se caiu; `detectarQueda(sales, hoje)` é pura/testável). Vendas diárias em `daily_sales` (unique listingId+date). Pipeline `backend/src/services/ai/salesDiagnosis.pipeline.ts` gera causa_provavel + evidencias + acoes_recomendadas + urgencia (cache 6h), enriquecido com contexto de concorrência (concorrente mais barato / buy box), via fila `salesDiagnosis.job.ts`. Endpoints: `POST /api/listings/:id/sales` (uma venda ou lote `{sales:[]}`), `GET /api/listings/:id/sales` (série + tendência), `GET /api/sales/trends` (todos, quedas primeiro), `POST /api/listings/:id/diagnosis` + `GET .../diagnosis/:jobId`. Tela `frontend/src/app/(dashboard)/sales-drop/`: SalesDropList (queda destacada), SalesTrendChart (área 90d), DiagnosisCard com UrgencyBadge colorido.

## Roadmap concluído
As 5 features prioritárias do produto estão construídas. Próximos passos naturais: OAuth do Mercado Livre por tenant, importação/sincronização real de anúncios (hoje as telas usam `listingId: 'demo'`), autenticação (`/api/auth/login`) e o consumidor da `notificationQueue`.
